...
class ExtensibleKVCache:
    ...
    def committed_kv_cache_tensors(
        ...
        Under block-outermost layouts the committed blocks are one prefix of the
        allocation, so the capacity placements still hold. Under layer-outermost
        layouts each layer's view has its own storage over its committed prefix:
        describe it as a one-layer tensor of that storage.
        """
        tensors = kv_cache_config.kv_cache_tensors
        ...
        for tensor in tensors:
            ...
            for layer_idx, layer_name in enumerate(tensor.layers):
                ...
                size = num_blocks * tensor.block_stride
                if not spec.has_layer_views:
                    # The layer keeps the whole reservation as its storage.
                    offset, size = start, self.size
                committed.append(
                    KVCacheTensor(
                        size=size,
                        layers=[layer_name],
